...
public class OrderDaoDB implements OrderDAO {
	...
	public void insertOrder(Order order) {
		...
		jdbcTemplate.update(UPDATE_PRODUCT_STOCK, product.getId(), product.getStock());

		KeyHolder keyHolder = new GeneratedKeyHolder();
		jdbcTemplate.update(Connection -> {
			PreparedStatement ps1 = Connection.prepareStatement(INSERT_ORDER, new String[] { "id" });
		...
		for (OrderItem item : order.getOrderItemSet()) {
			...
			jdbcTemplate.update(Connection -> {
				PreparedStatement ps2 = Connection.prepareStatement(INSERT_ORDER_ITEM, new String[] { "id" });
